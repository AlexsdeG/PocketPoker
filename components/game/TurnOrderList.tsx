...
export const TurnOrderList: React.FC = () => {
  ...
  const renderItem = (p: typeof players[0], status: 'prev' | 'current' | 'next') => {
      ...
      return (
          <div key={`${p.id}-${status}`} className={`flex items-center space-x-2 p-2 rounded-lg transition-all ${opacity} ${scale} ${border} border`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center ${p.isBot ? 'bg-white/10' : 'bg-brand-blue'}`}>
                 {p.avatarUrl ? (
                     <img src={p.avatarUrl} className="w-full h-full rounded-full object-cover"/>
                 ) : (
                     p.isBot ? <Bot size={14} color={p.color || '#fff'} /> : <User size={14} />
                 )}
              </div>
  ...
  return (
    <div className="absolute left-0 top-16 md:top-1/2 md:-translate-y-1/2 flex flex-col space-y-2 pointer-events-none z-50">
        <div className="bg-black/30 backdrop-blur-md p-2 rounded-r-xl border-y border-r border-white/5 shadow-xl pointer-events-auto">
            <div className="text-xs text-white/30 font-mono uppercase tracking-widest mb-2 hidden md:block px-1">Turn Order</div>
